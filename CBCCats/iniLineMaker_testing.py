...
def getSpinFromName(name):
    if name == "aligned":
        ...
        print(
            "Something went wrong in getSpinFromName function, name supplied: {}".format(
                name
            )
        )
...
def makeInjection_dict(injKeys, row):
    ...
    for k in injKeys:
        ...
        if k in ["ra"]:
            myDict[k] = np.deg2rad(
                row[k]
            )  # SkySim is over -180 to 180 deg, while Bilby defaults to 0 to 2pi
            # Originally had 'm' before here...
        elif k in ["dec"]:
            myDict[k] = np.deg2rad(
                row[k]
            )  # Skysim is from -90 to 90 deg, while bilby is -pi/2 to pi/2
            # Originally had 'm' before here...
        else:
            myDict[k] = row[k]
    gcTime = 1
...
basePath = "/pscratch/sd/s/seanmacb/proj411ProdRuns/testing"
...
# pd.read_csv()
msk = [
    (x.endswith("withSNRs_gwtc4.csv") and (x.__contains__("NSBH")))
    for x in os.listdir(dataDir)
]
msk = [
    (x.__contains__("uWeight")) and (x.__contains__("testing_4"))
    for x in os.listdir(dataDir)
]  # Modified for testing
# To include NSBH's, add an or statement to the second conditional
files = np.sort(
    np.array(os.listdir(dataDir))[msk]
)  # Only the last two entries, for the y band CBC's
...
for f in files:
    ...
    batchPath = os.path.join(basePath, "{}".format(f.split("gwtc4_")[-1].split(".")[0]))
    ...
    for index, row in subsampledDF.iterrows():
        ...
        toWrite = np.append(
            toWrite, [label, outDir, priorPath, injDict, injectionFPath, wfModel]
        )
    ...
    for writeables in toWrite:
        with open("{}_base_2048.ini".format(CBCType), "r") as myf:
            read = myf.readlines()
    ...
    for injectionDictionary, label, out in zip(
        toWrite[:, 3], toWrite[:, 0], toWrite[:, 1]
    ):
        with open(
            os.path.join(injectionFilePath, "{}_injection.dat".format(label)), "w"
        ) as myf:
    ...
    # labels_snrs_dict
    sortedDict = {
        k: v for k, v in sorted(labels_snrs_dict.items(), key=lambda item: item[1])
    }
    result = [k for k in list(sortedDict.keys())[9::10]]
    print("For {}".format(f))
    for r in result:
        print(r, sortedDict[r])
    print("Finished writing for {}, written to {}".format(f, batchPath), end="\n\n")
