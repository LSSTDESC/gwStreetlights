...
def priorSelector(alignment,CBCType):
    if alignment=="aligned":
        if CBCType=="BBH":
            return os.path.join(priorBasePath,"bbh_aligned_gwtc4_noMass.prior")  # Return the BBH aligned prior
        elif CBCType=="NSBH":
             return os.path.join(priorBasePath,"nsbh_aligned_gwtc4_noMass.prior")  # Return the NSBH aligned prior
        else:
        ...
        if CBCType=="BBH":
            return os.path.join(priorBasePath,"bbh_precessing_gwtc4_noMass.prior")  # Return the BBH precessing prior
        elif CBCType=="NSBH":
            return os.path.join(priorBasePath,"nsbh_precessing_gwtc4_noMass.prior")  # Return the NSBH precessing prior
        else:
            print("Something went wrong in the prior selector, inputs were alignment: {}. CBCType: {}".format(alignment,CBCType))
    else:
        print("Something went wrong in the prior selector, inputs were alignment: {}. CBCType: {}".format(alignment,CBCType))
    ...
    for cbcweight in CBCWeights:
        for cbctype in CBCTypes:
            for alignment in alignmentTypes:
                outFileName = os.path.join(dataDirectory,f"UniformParent,{cbcweight},{cbctype},{alignment},withCBCParams_gwtc4.csv") 
