...
for k in range(2, 5):
    ...
    psicbc = (
        lambda z: (1 + z) ** (1.82) / (1 + (z / 2) ** 3.82) * 1 / (1 + z)
    )  # eqn 15 from 2405.07904
    frate = lambda M, z, Mstar=-20.378, eps=1: 10 ** (
        -0.4 * eps * (M - Mstar)
    ) * psicbc(
        z
    )  # eqn 17 from 2405.07904
    ...
    _mgrid = np.linspace(
        -23, 0, 1000
    )  #  Mmax = 0, fixes the Nevents in 52.243 for z in (0,10)
    ...
    allQuantitiesOfInterest = np.array(
        [
            "ra",
            "ra_true",
            "dec",
            ...
            "LSST_filters/magnitude:LSST_g:rest",  # magnitudes in rest and obs frames
            ...
            "LSST_filters/totalLuminositiesStellar:LSST_u:observed",  # stellar luminosities in all bands, rest and obs frames
    ...
    filteredQuantities = np.append(
        allQuantitiesOfInterest[filterMask], "LSST_filters/magnitude:LSST_r:rest"
    )
    ...
    nn = 6000000
    plt.scatter(
        data["redshiftHubble"].values[:nn],
        data["LSST_filters/magnitude:LSST_r:rest"][:nn],
        c=np.log10(phost.values[:nn]),
    ...
    def stretch(
        ra_in,
        dec_in,
        ra_range=(60.46709862957921, 70.3338108507918),
        dec_range=(-46.57034567653332, -32.79850657784978),
    ):
        ...
        stretched_dec = (
            1000 * 180 * (delta_dec / (dec_range[1] - dec_range[0]))
        ) % 180 - 90
    ...
    events_index = np.random.choice(data.index, size=n_merges, p=phost.values)
    ...
    catmock = cat_cbc.apply(
        lambda x: stretch(x["ra"], x["dec"]), result_type="expand", axis=1
    )
    catmock = catmock.rename(
        columns={0: "mra", 1: "mdec"},
    )
    ...
    h, yedges, xedges, img = plt.hist2d(
        x=cat_cbc["mra"], y=cat_cbc["mdec"], bins=40, cmap=plt.get_cmap("magma", 20)
    )
    plt.xlim(0, 360)
    plt.ylim(-90, 90)
    cax = plt.colorbar()
