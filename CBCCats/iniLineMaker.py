...
for f in files:
    ...
    batchPath = os.path.join(
        basePath, "{},{}".format(parentWeightingDir, cbcWeightingDir)
    )
    ...
    for index, row in subsampledDF.iterrows():
        ...
        label = makeLabel(
            parentWeighting + "," + cbcWeighting, fullItem + _pass, alignment
        )
    ...
    for writeables in toWrite:
        with open("{}_base_1024.ini".format(CBCType), "r") as myf:
            read = myf.readlines()
