...
bb.gw.cosmology.DEFAULT_COSMOLOGY = FlatLambdaCDM(
...
def getGWTC4MassPrior(typ):
    if typ.lower() == "bbh":
        return gwcosmo.priors.BBH_broken_powerlaw_multi_peak_gaussian(
            alpha_1=1.7282865329473678,
            alpha_2=4.511690397723029,
            b=0.5,
            beta=1.1709777343204348,
            mminbh=5.058572081868678,
            mmaxbh=300,
            lambda_g=0.36102328981694193,
            lambda_g_low=0.5860680995810035,
            mu_g_low=9.763667347989355,
            sigma_g_low=0.6491643865532204,
            mu_g_high=32.76291758105389,
            sigma_g_high=3.9181194675793933,
            delta_m=4.320691590156577,
        )
    elif typ.lower() == "nsbh":
        ...
        nsPrior = bb.core.prior.TruncatedGaussian(
            1.4,
            0.68,
            0.1,
            3,
...
def getMassParamSample(mPrior):
    if type(mPrior) == gwcosmo.priors.BBH_broken_powerlaw_multi_peak_gaussian:
        ...
        raise ValueError(
            f"Provided prior ({mPrior}) is incorrect, something went wrong here..."
        )
    q = m2 / m1
    chrp = chirp(m1, m2)
    return m1, m2, q, chrp
...
def main(args):
    ...
    for ids, row in hostDF.iterrows():
        ...
        injDict["luminosity_distance"].append(
            float(
                str(
                    skysimCat.cosmology.luminosity_distance(row["redshiftHubble"])
                ).split(" ")[0]
            )
        )
    ...
    saveColumns = hostDF.columns.values[
        1:
    ]  # This is because of the extra column - check this always!!!
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--outFileName",
        type=str,
        default="BBHs_0,aligned,sampledOnly.csv",
        help="The name of the output csv, appended to the dataDir argument.",
    )
    parser.add_argument(
        "-t", "--cbcType", type=str, default="BBH", help="The CBC type to be sampled."
    )
